...
<let name="SUBTYPES-BT-5071-BT-5141-Part"
    value="('4', '5', '6')" />
...
<rule context="$ROOT-NODE/cac:ProcurementProjectLot[cbc:ID/@schemeName = ('Lot','Part')]/cac:ProcurementProject/cac:RealizedLocation/cac:Address">
...
<rule context="($EXTENSION-ORG-NODE/efac:TouchPoint union $EXTENSION-ORG-NODE/efac:Company)/cac:PostalAddress">
...
Because also ted example  https://github.com/OP-TED/eForms-SDK/blob/1.7.0/examples/notices/can_24_maximal.xml uses BT-735 in both places, we will check for this with AND logic. Leaving it open if one place will be skipped later. See laso https://github.com/OP-TED/eForms-SDK/discussions/596
...
<!-- context: Match BT-717. Mandatory is checked by cardinality rule: CR-DE-BT-717-Lot  -->
  <rule
    ...
    <assert
      id="SR-DE-33"
      test="exists($LOT-RESULT/efac:StrategicProcurement/efac:StrategicProcurementInformation)"
      role="error">[SR-DE-33] StrategicProcurementInformation must exist if cvd=BT-717=true</assert>
...
<let name="BT-717-MATCH-ID"
  ...
  <rule
    ...
    <let name="BT-723" value="efac:ProcurementDetails/efbc:AssetCategoryCode[@listName = 'vehicle-category']"/>

    <assert
      id="BR-DE-24-BT-723"
      test="exists($BT-723)"
      role="error">[BR-DE-24-BT-723] Statistics on at least one vehicle-category must exist here and in LotResult.</assert>

    <assert
      id="BR-DE-24-distinct"
      test="count($BT-723) = count(distinct-values($BT-723))"
      role="error">[BR-DE-24-distinct] Each vehicle category must be reported only once. But <value-of select="for $vcat in distinct-values($BT-723)
return concat($vcat, ' appears ', count($BT-723[. = $vcat]) )"/></assert>

  </rule>

  <rule context="$LOT-RESULT[
                efac:TenderLot/cbc:ID = $BT-717-MATCH-ID
    ...
    <assert
      ...
      role="error">[BR-DE-24-BT-715] Statistics on BT-715: vehicles must exist and values must be &gt;= 0</assert>

    <assert
      id="BR-DE-24-BT-725"
      test="$STATS[efbc:StatisticsCode[. = 'vehicles-zero-emission']]/efbc:StatisticsNumeric[ xs:integer(.) ge 0]"
      role="error">[BR-DE-24-BT-725] Statistics on BT-725: vehicles-zero-submission must exist and values must be &gt;= 0</assert>


    <assert
      id="BR-DE-24-BT-716"
      test="$STATS[efbc:StatisticsCode[. = 'vehicles-clean']]/efbc:StatisticsNumeric[ xs:integer(.) ge 0]"
      role="error">[BR-DE-24-BT-716] Statistics on BT-725: vehicles-zero-submission must exist and values must be &gt;= 0</assert>
  </rule>
...
<rule context="$ROOT-NODE/cac:ProcurementProjectLot[cbc:ID/@schemeName = ('Lot','Part')]/cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/cac:SubordinateAwardingCriterion">
...
<rule context="$EXTENSION-ORG-NODE-PARENT/efac:UltimateBeneficialOwner[cbc:ID/text()= $EXTENSION-ORG-NODE-PARENT/efac:Organization[efbc:ListedOnRegulatedMarketIndicator/text()=('false','0')]/efac:UltimateBeneficialOwner/cbc:ID/text()]">
